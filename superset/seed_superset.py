...
from superset_queries import DATASETS, DATASET_CONFIG, SAVED_QUERIES
...
def login():
    ...
    resp = _request("POST", "/api/v1/security/login", {
        "username": USERNAME,
        "password": PASSWORD,
        "provider": "db",
        "refresh": True,
...
def get_dataset_columns(ds_id):
    ...
    if ds_id not in _column_cache:
        ...
        _column_cache[ds_id] = {
            col["column_name"]: col for col in ds["result"]["columns"]
        }
...
def create_datasets(db_id):
    ...
    for name, sql in DATASETS.items():
        resp = _request("POST", "/api/v1/dataset/", {
            "database": db_id,
            "schema": "public",
            "table_name": name,
            "sql": sql,
...
def create_chart(name, viz_type, datasource_id, dashboard_id, params,
                  description=""):
    params.update({
...
def create_trends_charts(ds, dashboard_id):
    ...
    # Diversity over time
    ids.append(create_chart(
        "Name Diversity Over Time", "echarts_timeseries_line",
        ds["Name Diversity"], dashboard_id, {
            "x_axis": "year",
            "metrics": [_adhoc_metric(nd_cols["unique_names_per_m"], "MAX", "Unique names per 1M pop.")],
            "groupby": ["country"],
    ...
    ids.append(create_chart(
        "Top-10 Concentration Over Time", "echarts_timeseries_line",
        ds["Name Diversity"], dashboard_id, {
            "x_axis": "year",
    ...
    ids.append(create_chart(
        "'Nathan' Cross-Country", "echarts_timeseries_line",
        ds["Forename Trends Multi-Year"], dashboard_id, {
            "x_axis": "year",
    ...
    ids.append(create_chart(
        "'Sarah' Cross-Country", "echarts_timeseries_line",
        ds["Forename Trends Multi-Year"], dashboard_id, {
            "x_axis": "year",
            "metrics": [_adhoc_metric(ftmy_cols["pct_of_year_total"], "SUM", "% of births")],
            "groupby": ["country"],
    ...
    ids.append(create_chart(
        "Top Name Share Over Time", "echarts_timeseries_line",
        ds["Top Name Share"], dashboard_id, {
            "x_axis": "year",
    ...
    ids.append(create_chart(
        "Letter Distribution", "pivot_table_v2",
        ds["Letter Distribution"], dashboard_id, {
...
def create_snapshot_charts(ds, dashboard_id):
    ...
    ids.append(create_chart(
        "Top 10 US Forenames", "echarts_timeseries_bar",
        ds["Forename Rankings"], dashboard_id, {
            "x_axis": "name",
    ...
    ids.append(create_chart(
        "Top 10 US Surnames", "echarts_timeseries_bar",
        ds["Surname Rankings"], dashboard_id, {
            "x_axis": "name",
    ...
    ids.append(create_chart(
        "Most Gender-Neutral Names (by share of births)", "table",
        ds["Gender-Neutral Names"], dashboard_id, {
            "all_columns": ["name", "country", "male_count", "female_count",
                            "pct_of_births", "balance_pct"],
            "adhoc_filters": [],
            "row_limit": 1000,
            "order_desc": True,
    ...
    ids.append(create_chart(
        "Cross-Country Names", "table",
        ds["Cross-Country Names"], dashboard_id, {
    ...
    ids.append(create_chart(
        "Country Signature Names", "table",
        ds["Country Signature Names"], dashboard_id, {
            "all_columns": ["name", "gender", "country", "local_pct",
                            "global_avg_pct", "distinctiveness_ratio"],
            "adhoc_filters": [],
            "row_limit": 1000,
            "order_desc": True,
    ...
    # Big number totals
    ids.append(create_chart(
        "Total Distinct Forenames", "big_number_total",
        ds["Forename Trends"], dashboard_id, {
    ...
    ids.append(create_chart(
        "Total Distinct Surnames", "big_number_total",
        ds["Surname Trends"], dashboard_id, {
...
def create_validation_charts(ds, dashboard_id):
    ...
    # Raw source health
    ids.append(create_chart(
        "Raw Source Row Counts", "echarts_timeseries_bar",
        ds["Raw Source Counts"], dashboard_id, {
            "x_axis": "source",
    ...
    ids.append(create_chart(
        "Empty Sources (Alert)", "table",
        ds["Empty Sources"], dashboard_id, {
    ...
    # Volume by country
    ids.append(create_chart(
        "Forenames by Country & Gender", "echarts_timeseries_bar",
        ds["Forenames by Country"], dashboard_id, {
            "x_axis": "country",
    ...
    ids.append(create_chart(
        "Forename Year Coverage", "pivot_table_v2",
        ds["Forename Year Coverage"], dashboard_id, {
            "groupbyColumns": ["year"],
            "groupbyRows": ["country"],
    ...
    ids.append(create_chart(
        "Source Quality Scorecard", "table",
        ds["Source Quality Scorecard"], dashboard_id, {
            "all_columns": [
                "source_id", "row_count", "ingest_success_count",
                "ingest_fail_count",
                "last_run_at", "last_success_rows",
            ],
            "adhoc_filters": [],
            "row_limit": 50,
            "order_desc": False,
...
def create_dashboard(title, slug, rows, chart_names):
    ...
    """
    resp = _request("POST", "/api/v1/dashboard/", {
    ...
    _request("PUT", f"/api/v1/dashboard/{dash_id}", {
...
def main():
    ...
    trends_dash = _request("POST", "/api/v1/dashboard/", {
    ...
    _request("PUT", f"/api/v1/dashboard/{trends_dash}", {
        "position_json": json.dumps(position),
        "json_metadata": json.dumps(metadata),
    })
    ...
    snapshots_dash = _request("POST", "/api/v1/dashboard/", {
    ...
    _request("PUT", f"/api/v1/dashboard/{snapshots_dash}", {
        "position_json": json.dumps(position),
        "json_metadata": json.dumps(metadata),
    })
    ...
    val_dash = _request("POST", "/api/v1/dashboard/", {
    ...
    _request("PUT", f"/api/v1/dashboard/{val_dash}", {
        "position_json": json.dumps(position),
        "json_metadata": json.dumps(metadata),
    })
